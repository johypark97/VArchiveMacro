...
- 스캐너
  ...
  - 비인식 모드
    ...
    - 주의 사항: **사용 조건을 갖추었더라도 렉 또는 키 입력 무시가 발생하면 기록이 잘못
      올라갈 가능성이 있으므로 업로드 전 항상 확인이 필요합니다.**
...
1. **비인식 모드 주의사항**
    ...
    캡처한 것으로 볼 수 있습니다. 그렇지 않다면 해당 탭은 캡처가 밀린 것이므로 업로드를 하면
    안됩니다.

    **또한 모든 사용 조건을 만족하여 정상적으로 캡처를 완료 하였더라도 게임의 화면 업데이트
    지연 또는 키 입력 무시와 같은 문제가 매우 짧은 순간에 발생하면 곡 매칭이 밀려 잘못
    기록이 업로드될 수 있습니다. 때문에 업로드 전 항상 주의가 필요합니다.**
